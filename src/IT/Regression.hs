...
--  | otherwise    = Nothing
  ...
  where
    ...
    res         = tryToRound (fitFun ys) zss <$> regress ys zss
    (ysHat, ws) = fromJust res
    fit         = NE.toList $ NE.map ((`uncurry` (ys, ysHat)) . _fun) measures
    ps          = Sol expr fit ws
...
-- It was: fitnessTest
evalTest :: NonEmpty Measure -> Dataset Double -> Vector -> Solution Double -> Maybe [Double]
